...
static void concealBlocks( int lastColumn, int lastRow, int comp, frame *recfr, int32 picSizeX, int *condition,  h264_decoder* dec_params );
static void pixMeanInterpolateBlock( imgpel *src[], imgpel *block, int blockSize, int frameWidth ,  h264_decoder* dec_params);
 ...
 int ercConcealIntraFrame( frame *recfr, int32 picSizeX, int32 picSizeY, ercVariables_t *errorVar,  h264_decoder* dec_params ) 
{
  ...
  if ( errorVar && errorVar->concealment ) 
  {
    ...
    { 
      ...
      concealBlocks( lastColumn, lastRow, 0, recfr, picSizeX, errorVar->yCondition,dec_params );
      
      /* U (dimensions halved compared to Y) */
      lastRow = (int) (picSizeY>>4);
      lastColumn = (int) (picSizeX>>4);
      concealBlocks( lastColumn, lastRow, 1, recfr, picSizeX, errorVar->uCondition,dec_params );
      
      /* V ( dimensions equal to U ) */
      concealBlocks( lastColumn, lastRow, 2, recfr, picSizeX, errorVar->vCondition,dec_params );
    }
    return 1;
  }
  else
 ...
 ************************************************************************
 */
void ercPixConcealIMB(imgpel *currFrame, int row, int column, int predBlocks[], int frameWidth, int mbWidthInBlocks,  h264_decoder* dec_params) 
{
   ...
   pixMeanInterpolateBlock( src, currBlock, mbWidthInBlocks*8, frameWidth,dec_params );
      ...
      if ( condition[row*lastColumn+column] <= thr ) 
      {
        ...
        if ( lastCorruptedRow >= lastRow ) 
        {
          ...
          for( currRow = firstCorruptedRow; currRow < lastRow; currRow += step ) 
          {
            ...
            {
            case 0 :
              ercPixConcealIMB( recfr->yptr, currRow, column, predBlocks, picSizeX, 2 ,dec_params);
              break;
            case 1 :
              ercPixConcealIMB( recfr->uptr, currRow, column, predBlocks, (picSizeX>>1), 1,dec_params );
              break;
            case 2 :
              ercPixConcealIMB( recfr->vptr, currRow, column, predBlocks, (picSizeX>>1), 1,dec_params );
              break;
            }
  ...
  for( row = 0; row < blockSize; row++ ) 
  {
    for( column = 0; column < blockSize; column++ ) 
    {
      ...
      if ( srcCounter > 0 )
        ...
        block[ k + column ] = DC_PRED_VALUE;   //FREXT
    }
    k += frameWidth;
  }
