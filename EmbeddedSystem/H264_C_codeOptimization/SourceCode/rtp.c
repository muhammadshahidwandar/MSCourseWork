 ...
 ************************************************************************
 */
void OpenRTPFile ( char *fn,  h264_decoder* dec_params )
{
	if (NULL == (dec_params->bits_Annexb=fopen(fn, "rb")))
	{
		printf ("Cannot open RTP file '%s'\n", dec_params->input->infile);
		exit(0);
	}
}
 ...
 ************************************************************************
 */
void CloseRTPFile(h264_decoder* dec_params)
{
...
int GetRTPNALU( NALU_t *nalu,  h264_decoder* dec_params )
{
  RTPpacket_t *p;
  int ret;
  ...
  ret = RTPReadPacket (p, dec_params->bits_Annexb);
