...
//static int ec_flag[SE_MAX_ELEMENTS]; /*Changed by Saad Bin Shams [Removing Global Variables]*/
/*
...

{
  ...
  */
  ...
  {
  case SE_HEADER :
    dec_params->ec_flag[SE_HEADER] = EC_REQ;
  case SE_PTYPE :
    dec_params->ec_flag[SE_PTYPE] = EC_REQ;
  case SE_MBTYPE :
    dec_params->ec_flag[SE_MBTYPE] = EC_REQ;

  case SE_REFFRAME :
    dec_params->ec_flag[SE_REFFRAME] = EC_REQ;
    dec_params->ec_flag[SE_MVD] = EC_REQ; // set all motion vectors to zero length
    se = SE_CBP_INTER;      // conceal also Inter texture elements
    break;

  case SE_INTRAPREDMODE :
    dec_params->ec_flag[SE_INTRAPREDMODE] = EC_REQ;
    se = SE_CBP_INTRA;      // conceal also Intra texture elements
    break;
  case SE_MVD :
    dec_params->ec_flag[SE_MVD] = EC_REQ;
    se = SE_CBP_INTER;      // conceal also Inter texture elements
    break;
  ...
  {
  case SE_CBP_INTRA :
    dec_params->ec_flag[SE_CBP_INTRA] = EC_REQ;
  case SE_LUM_DC_INTRA :
    dec_params->ec_flag[SE_LUM_DC_INTRA] = EC_REQ;
  case SE_CHR_DC_INTRA :
    dec_params->ec_flag[SE_CHR_DC_INTRA] = EC_REQ;
  case SE_LUM_AC_INTRA :
    dec_params->ec_flag[SE_LUM_AC_INTRA] = EC_REQ;
  case SE_CHR_AC_INTRA :
    dec_params->ec_flag[SE_CHR_AC_INTRA] = EC_REQ;
    break;

  case SE_CBP_INTER :
    dec_params->ec_flag[SE_CBP_INTER] = EC_REQ;
  case SE_LUM_DC_INTER :
    dec_params->ec_flag[SE_LUM_DC_INTER] = EC_REQ;
  case SE_CHR_DC_INTER :
    dec_params->ec_flag[SE_CHR_DC_INTER] = EC_REQ;
  case SE_LUM_AC_INTER :
    dec_params->ec_flag[SE_LUM_AC_INTER] = EC_REQ;
  case SE_CHR_AC_INTER :
    dec_params->ec_flag[SE_CHR_AC_INTER] = EC_REQ;
    break;
  case SE_DELTA_QUANT_INTER :
    dec_params->ec_flag[SE_DELTA_QUANT_INTER] = EC_REQ;
    break;
  case SE_DELTA_QUANT_INTRA :
    dec_params->ec_flag[SE_DELTA_QUANT_INTRA] = EC_REQ;
    break;
  default:
    break;
...
{
  if (dec_params->ec_flag[sym->type] == NO_EC)
    return NO_EC;
/*
#if TRACE
