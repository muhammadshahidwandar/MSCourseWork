...
/*************************************************
Changes to function and structure for now
type = img->type
qp = img->qp
*************************************************/
#define BIARI_CTX_INIT2(ii,jj,ctx,tab,num,type,qp) \
{ \
    for (i=0; i<ii; i++) \
{ \
    for (j=0; j<jj; j++) \
...
#define BIARI_CTX_INIT1(jj,ctx,tab,num,type,qp) \
{ \
    for (j=0; j<jj; j++) \
{ \
    if((type==I_SLICE)||(type==SI_SLICE)) \
{ \
    biari_init_context (qp, &(ctx[j]), &(tab ## _I[num][0][j][0])); \
} \
    else \
{ \
    biari_init_context (qp, &(ctx[j]), &(tab ## _P[num][0][j][0])); \
} \
} \
}
/*************************************************
...
/***********************************************************************************\
* FUNCTION     : create_contexts_MotionInfo.                                        *
*-----------------------------------------------------------------------------------*
* DISCRIPTION  : Allocation of contexts models for the motion info used for         *
*              : arithmetic decoding                                                *
*-----------------------------------------------------------------------------------*
* ARGUMENTS    : Decoder Parameters.                                                *
*              : Decoder Outputs.                                                   *
*-----------------------------------------------------------------------------------*
* RETURN VALUE : Address allocated space.                                           *
*-----------------------------------------------------------------------------------*
* OUTPUT       : NONE                                                               *
*-----------------------------------------------------------------------------------*
...
MotionInfoContexts* create_contexts_MotionInfo()
{
    ...
    deco_ctx = (MotionInfoContexts*) h264_calloc(1, sizeof(MotionInfoContexts) );
    if( deco_ctx == NULL )
    {
		printf("create_contexts_MotionInfo: deco_ctx");
		exit(0);
	}
...
/***********************************************************************************\
* FUNCTION     : create_contexts_TextureInfo.                                       *
*-----------------------------------------------------------------------------------*
* DISCRIPTION  : Allocation of contexts models for the testure info used for        *
*              : arithmetic decoding                                                *
*-----------------------------------------------------------------------------------*
* ARGUMENTS    : Decoder Parameters.                                                *
*              : Decoder Outputs.                                                   *
...
TextureInfoContexts* create_contexts_TextureInfo()
{
    ...
    deco_ctx = (TextureInfoContexts*) h264_calloc(1, sizeof(TextureInfoContexts) );
    if( deco_ctx == NULL )
    {
		printf("create_contexts_TextureInfo: deco_ctx");
		exit(0);
	}
...
/***********************************************************************************\
* FUNCTION     : delete_contexts_MotionInfo.                                        *
*-----------------------------------------------------------------------------------*
* DISCRIPTION  : Frees the memory of the contexts models used for arithmetic        *
*              : decoding of the motion info.                                       *
*-----------------------------------------------------------------------------------*
* ARGUMENTS    : Contexts models pointer to be freed.                               *
*-----------------------------------------------------------------------------------*
* RETURN VALUE : NONE                                                               *
*-----------------------------------------------------------------------------------*
* OUTPUT       : NONE                                                               *
...
/***********************************************************************************\
* FUNCTION     : delete_contexts_TextureInfo.                                       *
*-----------------------------------------------------------------------------------*
* DISCRIPTION  : Frees the memory of the contexts models used for arithmetic        *
*              : decoding of the texture info.                                      *
*-----------------------------------------------------------------------------------*
* ARGUMENTS    : Contexts models pointer to be freed.                               *
*-----------------------------------------------------------------------------------*
* RETURN VALUE : NONE                                                               *
...
*-----------------------------------------------------------------------------------*
* DISCRIPTION  : Initialize the contexts models used for arithmetic decoding.       *
*-----------------------------------------------------------------------------------*
* ARGUMENTS    : Decoder Parameters.                                                *
*-----------------------------------------------------------------------------------*
* RETURN VALUE : NONE                                                               *
...
*              : <saad.shams@inforient.com>                                         *
\***********************************************************************************/
void init_contexts (h264_decoder* dec_params)
{
    MotionInfoContexts*  mc = dec_params->mot_ctx;
    TextureInfoContexts* tc = dec_params->tex_ctx;
    ...
    BIARI_CTX_INIT2 (3, NUM_MB_TYPE_CTX,   mc->mb_type_contexts,     INIT_MB_TYPE,    model_number, type, qp);
    BIARI_CTX_INIT2 (2, NUM_B8_TYPE_CTX,   mc->b8_type_contexts,     INIT_B8_TYPE,    model_number, type, qp);
    BIARI_CTX_INIT2 (2, NUM_MV_RES_CTX,    mc->mv_res_contexts,      INIT_MV_RES,     model_number, type, qp);
    BIARI_CTX_INIT2 (2, NUM_REF_NO_CTX,    mc->ref_no_contexts,      INIT_REF_NO,     model_number, type, qp);
    BIARI_CTX_INIT1 (   NUM_DELTA_QP_CTX,  mc->delta_qp_contexts,    INIT_DELTA_QP,   model_number, type, qp);
    BIARI_CTX_INIT1 (   NUM_MB_AFF_CTX,    mc->mb_aff_contexts,      INIT_MB_AFF,     model_number, type, qp);
    BIARI_CTX_INIT1 (   NUM_TRANSFORM_SIZE_CTX,  mc->transform_size_contexts,    INIT_TRANSFORM_SIZE,   model_number, type, qp);
    
    
    //--- texture coding contexts ---
    BIARI_CTX_INIT1 (                 NUM_IPR_CTX,  tc->ipr_contexts,     INIT_IPR,       model_number, type, qp);
    BIARI_CTX_INIT1 (                 NUM_CIPR_CTX, tc->cipr_contexts,    INIT_CIPR,      model_number, type, qp);
    BIARI_CTX_INIT2 (3,               NUM_CBP_CTX,  tc->cbp_contexts,     INIT_CBP,       model_number, type, qp);
    BIARI_CTX_INIT2 (NUM_BLOCK_TYPES, NUM_BCBP_CTX, tc->bcbp_contexts,    INIT_BCBP,      model_number, type, qp);
    BIARI_CTX_INIT2 (NUM_BLOCK_TYPES, NUM_MAP_CTX,  tc->map_contexts,     INIT_MAP,       model_number, type, qp);
    BIARI_CTX_INIT2 (NUM_BLOCK_TYPES, NUM_LAST_CTX, tc->last_contexts,    INIT_LAST,      model_number, type, qp);
    BIARI_CTX_INIT2 (NUM_BLOCK_TYPES, NUM_ONE_CTX,  tc->one_contexts,     INIT_ONE,       model_number, type, qp);
    BIARI_CTX_INIT2 (NUM_BLOCK_TYPES, NUM_ABS_CTX,  tc->abs_contexts,     INIT_ABS,       model_number, type, qp);
    BIARI_CTX_INIT2 (NUM_BLOCK_TYPES, NUM_MAP_CTX,  tc->fld_map_contexts, INIT_FLD_MAP,   model_number, type, qp);
    BIARI_CTX_INIT2 (NUM_BLOCK_TYPES, NUM_LAST_CTX, tc->fld_last_contexts,INIT_FLD_LAST,  model_number, type, qp);
}
...
/***********************************************************************************\
* FUNCTION     : arideco_create_decoding_environment.                               *
*-----------------------------------------------------------------------------------*
* DISCRIPTION  : Allocates memory for the DecodingEnvironment struct.               *
*-----------------------------------------------------------------------------------*
* ARGUMENTS    : Decoder Parameters.                                                *
*-----------------------------------------------------------------------------------*
* RETURN VALUE : Address allocated space.                                           *
...
/***********************************************************************************\
* FUNCTION     : arideco_delete_decoding_environment.                               *
*-----------------------------------------------------------------------------------*
* DISCRIPTION  : Frees memory of the DecodingEnvironment struct.                    *
*-----------------------------------------------------------------------------------*
* ARGUMENTS    : Pointer to the environment structure to be freed.                  *
*              : Decoder Parameters.                                                *
*              : Decoder Outputs.                                                   *
*-----------------------------------------------------------------------------------*
* RETURN VALUE : NONE                                                               *
...
void arideco_delete_decoding_environment(DecodingEnvironmentPtr dep)
{
    if (dep == NULL)
    {
        printf("Error freeing dep (NULL pointer)");
    }
    else
    {
        free(dep);
...
/***********************************************************************************\
* FUNCTION     : arideco_start_decoding.                                            *
*-----------------------------------------------------------------------------------*
* DISCRIPTION  : Initializes the DecodingEnvironment for the arithmetic coder.      *
*-----------------------------------------------------------------------------------*
* ARGUMENTS    :                                                                    *
*              :                                                                    *
*              :                                                                    *
    ...
    *dep->Dcodestrm_len = firstbyte;    
    {
        ...
        {
            if (--dep->Dbits_to_go < 0)
            {
                dep->Dbuffer = dep->Dcodestrm[(*dep->Dcodestrm_len)++];
                dep->Dbits_to_go = 7;
                
            }
            value = (value<<1)  | ((dep->Dbuffer >> dep->Dbits_to_go) & 0x01);
        }
    }
    dep->Drange = HALF-2;
    dep->Dvalue = value;
...
/***********************************************************************************\
* FUNCTION     : arideco_bits_read.                                                 *
*-----------------------------------------------------------------------------------*
* DISCRIPTION  : Calculate the number of bits read.                                 *
*-----------------------------------------------------------------------------------*
* ARGUMENTS    :                                                                    *
*-----------------------------------------------------------------------------------*
...
*-----------------------------------------------------------------------------------*
\***********************************************************************************/
int arideco_bits_read(DecodingEnvironmentPtr dep_dp)
{
    return 8 * ((*dep_dp->Dcodestrm_len)-1) + (8 - dep_dp->Dbits_to_go) - 16;
}


/***********************************************************************************\
* FUNCTION     : arideco_done_decoding.                                             *
*-----------------------------------------------------------------------------------*
* DISCRIPTION  : Calculate the number of bits read.                                 *
*-----------------------------------------------------------------------------------*
* ARGUMENTS    :                                                                    *
...
/***********************************************************************************\
* FUNCTION     : biari_decode_symbol.                                               *
*-----------------------------------------------------------------------------------*
* DISCRIPTION  : Decode symbols.                                                    *
*-----------------------------------------------------------------------------------*
* ARGUMENTS    :                                                                    *
*              :                                                                    *
*-----------------------------------------------------------------------------------*
* RETURN VALUE : Decoded symbol.                                                    *
*-----------------------------------------------------------------------------------*
* OUTPUT       : NONE                                                               *
*-----------------------------------------------------------------------------------*
* CHANGE LOG   : Below are the changes done to the function                         *
...
/***********************************************************************************\
* FUNCTION     : biari_decode_symbol_eq_prob.                                       *
*-----------------------------------------------------------------------------------*
* DISCRIPTION  : Decode symbols.                                                    *
*-----------------------------------------------------------------------------------*
* ARGUMENTS    :                                                                    *
...
unsigned int biari_decode_symbol_eq_prob(DecodingEnvironmentPtr dep)
{
    ...
    /* Shift in next bit and add to value */  
    value |= (dep->Dbuffer >> dep->Dbits_to_go) &  0x01;  
    if (value >= dep->Drange) 
    {
        bit = 1;
        value -= dep->Drange;
...
/***********************************************************************************\
* FUNCTION     : biari_decode_symbol_final.                                         *
*-----------------------------------------------------------------------------------*
* DISCRIPTION  : Decode symbols.                                                    *
*-----------------------------------------------------------------------------------*
* ARGUMENTS    :                                                                    *
...
*-----------------------------------------------------------------------------------*
\***********************************************************************************/
unsigned int biari_decode_final(DecodingEnvironmentPtr dep_dp)
{
    register unsigned int value  = dep_dp->Dvalue;
    register unsigned int range  = dep_dp->Drange - 2;
    
#if (2==TRACE)
    fprintf(p_trace, "%d  0x%04x\n", binCount++, dep_dp->Drange );
#endif
    ...
    else
    {
        while (range < QUARTER)
        {
            /* Double range */
            range <<= 1;
            if (--dep_dp->Dbits_to_go < 0)
            {
                dep_dp->Dbuffer = dep_dp->Dcodestrm[(*dep_dp->Dcodestrm_len)++];
                dep_dp->Dbits_to_go = 7;
            }
            /* Shift in next bit and add to value */
            value = (value << 1) | ((dep_dp->Dbuffer >> dep_dp->Dbits_to_go) & 0x01);
        } 
        dep_dp->Dvalue = value;
        dep_dp->Drange = range;
        return 0;
...
/************************************************************************************\
* FUNCTION     : biari_init_context.                                                 *
*------------------------------------------------------------------------------------*
* DISCRIPTION  : Initializes a given context with some pre-defined probability state.*
*------------------------------------------------------------------------------------*
* ARGUMENTS    : Quantization Parameter.                                             *
*              :                                                                     *
*              :                                                                     *
*------------------------------------------------------------------------------------*
* RETURN VALUE : Decoded symbol.                                                     *
