...
//extern seq_parameter_set_rbsp_t SeqParSet[MAXSPS];/*Changed by Saad Bin Shams [Removing Global Variables]*/
 ...
 ************************************************************************
 */
void InterpretSEIMessage( byte* msg, int size, ImageParameters *img,  h264_decoder* dec_params )
{
  int payload_type = 0;
  int payload_size = 0;
  int offset = 1;
  ...
  do
  {
    ...
    case  SEI_BUFFERING_PERIOD:
      interpret_buffering_period_info( msg+offset, payload_size, img,dec_params);
      break;
    case  SEI_PIC_TIMING:
      interpret_picture_timing_info( msg+offset, payload_size, img, dec_params);
      break;
    case  SEI_PAN_SCAN_RECT:
      interpret_pan_scan_rect_info( msg+offset, payload_size, img, dec_params );
      break;
    ...
    case  SEI_RECOVERY_POINT:
      interpret_recovery_point_info( msg+offset, payload_size, img,dec_params );
      break;
    case  SEI_DEC_REF_PIC_MARKING_REPETITION:
      interpret_dec_ref_pic_marking_repetition_info( msg+offset, payload_size, img,dec_params );
      break;
    case  SEI_SPARE_PIC:
      interpret_spare_pic( msg+offset, payload_size, img,dec_params );
      break;
    case  SEI_SCENE_INFO:
      interpret_scene_information( msg+offset, payload_size, img,dec_params );
      break;
    case  SEI_SUB_SEQ_INFO:
      interpret_subsequence_info( msg+offset, payload_size, img,dec_params );
      break;
    case  SEI_SUB_SEQ_LAYER_CHARACTERISTICS:
      interpret_subsequence_layer_characteristics_info( msg+offset, payload_size, img,dec_params );
      break;
    case  SEI_SUB_SEQ_CHARACTERISTICS:
      interpret_subsequence_characteristics_info( msg+offset, payload_size, img,dec_params );
      break;
    ...
    case  SEI_FULL_FRAME_SNAPSHOT:
      interpret_full_frame_snapshot_info( msg+offset, payload_size, img,dec_params );
      break;
    case  SEI_PROGRESSIVE_REFINEMENT_SEGMENT_START:
      interpret_progressive_refinement_end_info( msg+offset, payload_size, img,dec_params );
      break;
    case  SEI_PROGRESSIVE_REFINEMENT_SEGMENT_END:
      interpret_progressive_refinement_end_info( msg+offset, payload_size, img ,dec_params);
      break;
    case  SEI_MOTION_CONSTRAINED_SLICE_GROUP_SET:
      interpret_motion_constrained_slice_group_set_info( msg+offset, payload_size, img,dec_params );
      break;
    default:
 ...
 ************************************************************************
*/
void interpret_spare_pic( byte* payload, int size, ImageParameters *img,   h264_decoder* dec_params )
{
  int i,x,y;
  Bitstream* buf;
  ...
  buf = (Bitstream *)h264_malloc(sizeof(Bitstream));
  buf->bitstream_length = size;
  buf->streamBuffer = payload;
  buf->frame_bitoffset = 0;

  target_frame_num = ue_v("SEI: target_frame_num", buf,dec_params);

#ifdef WRITE_MAP_IMAGE
  printf( "target_frame_num is %d\n", target_frame_num );
#endif

  num_spare_pics = 1 + ue_v("SEI: num_spare_pics_minus1", buf,dec_params);
  ...
  for (i=0; i<num_spare_pics; i++)
  {
    ...
    delta_spare_frame_num = ue_v("SEI: delta_spare_frame_num", buf,dec_params);

    SpareFrameNum = CandidateSpareFrameNum - delta_spare_frame_num;
    if( SpareFrameNum < 0 )
      SpareFrameNum = MAX_FN + SpareFrameNum;

    ref_area_indicator = ue_v("SEI: ref_area_indicator", buf,dec_params);
    ...
    case 1:   // The map is not compressed
      for (y=0; y<img->height/16; y++)
        for (x=0; x<img->width/16; x++)
        {
          map[i][y][x] = u_1("SEI: ref_mb_indicator", buf,dec_params);
        }
      break;
    case 2:   // The map is compressed
      ...
      for (m=0; m<img->height/16; m++)
        for (n=0; n<img->width/16; n++)
        {
          ...
          if (no_bit0<0)
          {
            no_bit0 = ue_v("SEI: zero_run_length", buf,dec_params);
          }
  ...
  {
    old_pn = img->number;
    get_mem2Dpel(&Y, img->height, img->width,dec_params);
    if (first)
    {
    ...
    for (kk=0; kk<num_spare_pics; kk++)
    {
      ...
      for (i=0; i < img->height; i++)
        for (j=0; j < img->width; j++)
          fwrite(&(Y[i][j]), symbol_size_in_bytes, 1, dec_params->p_out);
        
      for (k=0; k < 2; k++)
        for (i=0; i < img->height/2; i++)
          for (j=0; j < img->width/2; j++)
            fwrite(&(img->dc_pred_value), symbol_size_in_bytes, 1, dec_params->p_out);
    }
    fclose( fp );
    free_mem2Dpel( Y );
  }
 ...
 ************************************************************************
 */
void interpret_subsequence_info( byte* payload, int size, ImageParameters *img,   h264_decoder* dec_params )
{
  Bitstream* buf;
  ...
  sub_seq_layer_num        = ue_v("SEI: sub_seq_layer_num"       , buf,dec_params);
  sub_seq_id               = ue_v("SEI: sub_seq_id"              , buf,dec_params);
  first_ref_pic_flag       = u_1 ("SEI: first_ref_pic_flag"      , buf,dec_params);
  leading_non_ref_pic_flag = u_1 ("SEI: leading_non_ref_pic_flag", buf,dec_params);
  last_pic_flag            = u_1 ("SEI: last_pic_flag"           , buf,dec_params);
  sub_seq_frame_num_flag   = u_1 ("SEI: sub_seq_frame_num_flag"  , buf,dec_params);
  if (sub_seq_frame_num_flag)
  {
    sub_seq_frame_num        = ue_v("SEI: sub_seq_frame_num"       , buf,dec_params);
  }
 ...
 ************************************************************************
 */
void interpret_subsequence_layer_characteristics_info( byte* payload, int size, ImageParameters *img,   h264_decoder* dec_params )
{
  Bitstream* buf;
  ...
  num_sub_layers = 1 + ue_v("SEI: num_sub_layers_minus1", buf,dec_params);
  ...
  {
    accurate_statistics_flag = u_1(   "SEI: accurate_statistics_flag", buf,dec_params);
    average_bit_rate         = u_v(16,"SEI: average_bit_rate"        , buf,dec_params);
    average_frame_rate       = u_v(16,"SEI: average_frame_rate"      , buf,dec_params);
 ...
 ************************************************************************
 */
void interpret_subsequence_characteristics_info( byte* payload, int size, ImageParameters *img,   h264_decoder* dec_params )
{
  Bitstream* buf;
  int i;
  ...
  sub_seq_layer_num = ue_v("SEI: sub_seq_layer_num", buf,dec_params);
  sub_seq_id        = ue_v("SEI: sub_seq_id", buf,dec_params);
  duration_flag     = u_1 ("SEI: duration_flag", buf,dec_params);
  ...
  if ( duration_flag )
  {
    sub_seq_duration = u_v (32, "SEI: duration_flag", buf,dec_params);
#ifdef PRINT_SUBSEQUENCE_CHAR
    printf("sub_seq_duration = %ld\n", sub_seq_duration);
#endif
  }

  average_rate_flag = u_1 ("SEI: average_rate_flag", buf,dec_params);
  ...
  num_referenced_subseqs  = ue_v("SEI: num_referenced_subseqs", buf,dec_params);
  ...
  {
    ref_sub_seq_layer_num  = ue_v("SEI: ref_sub_seq_layer_num", buf,dec_params);
    ref_sub_seq_id         = ue_v("SEI: ref_sub_seq_id", buf,dec_params);
    ref_sub_seq_direction  = u_1 ("SEI: ref_sub_seq_direction", buf,dec_params);
 ...
 ************************************************************************
 */
void interpret_scene_information( byte* payload, int size, ImageParameters *img,   h264_decoder* dec_params )
{
  Bitstream* buf;
  ...
  scene_id              = ue_v("SEI: scene_id"             , buf,dec_params);
  scene_transition_type = ue_v("SEI: scene_transition_type", buf,dec_params);
  if ( scene_transition_type > 3 )
  {
    second_scene_id     = ue_v("SEI: scene_transition_type", buf,dec_params);;
  }
 ...
 ************************************************************************
 */
void interpret_pan_scan_rect_info( byte* payload, int size, ImageParameters *img,  h264_decoder* dec_params)
{
  ...
  pan_scan_rect_id = ue_v("SEI: pan_scan_rect_id", buf,dec_params);

  pan_scan_rect_left_offset   = se_v("SEI: pan_scan_rect_left_offset"  , buf, dec_params);
  pan_scan_rect_right_offset  = se_v("SEI: pan_scan_rect_right_offset" , buf, dec_params);
  pan_scan_rect_top_offset    = se_v("SEI: pan_scan_rect_top_offset"   , buf, dec_params);
  pan_scan_rect_bottom_offset = se_v("SEI: pan_scan_rect_bottom_offset", buf, dec_params);
 ...
 ************************************************************************
 */
void interpret_recovery_point_info( byte* payload, int size, ImageParameters *img,   h264_decoder* dec_params )
{
  ...
  recovery_frame_cnt       = ue_v(    "SEI: recovery_frame_cnt"      , buf,dec_params);
  exact_match_flag         = u_1 (    "SEI: exact_match_flag"        , buf,dec_params);
  broken_link_flag         = u_1 (    "SEI: broken_link_flag"        , buf,dec_params);
  changing_slice_group_idc = u_v ( 2, "SEI: changing_slice_group_idc", buf,dec_params);
 ...
 ************************************************************************
 */
void interpret_dec_ref_pic_marking_repetition_info( byte* payload, int size, ImageParameters *img ,   h264_decoder* dec_params)
{
  ...
  original_idr_flag     = u_1 (    "SEI: original_idr_flag"    , buf,dec_params);
  original_frame_num    = ue_v(    "SEI: original_frame_num"   , buf,dec_params);
  ...
  dec_ref_pic_marking(buf,dec_params);
 ...
 ************************************************************************
 */
void interpret_full_frame_snapshot_info( byte* payload, int size, ImageParameters *img ,  h264_decoder* dec_params)
{
  int snapshot_id;
  ...
  snapshot_id = ue_v("SEI: snapshot_id", buf,dec_params);
 ...
 ************************************************************************
 */
void interpret_progressive_refinement_start_info( byte* payload, int size, ImageParameters *img,   h264_decoder* dec_params )
{
  ...
  progressive_refinement_id   = ue_v("SEI: progressive_refinement_id"  , buf,dec_params);
  num_refinement_steps_minus1 = ue_v("SEI: num_refinement_steps_minus1", buf,dec_params);
 ...
 ************************************************************************
 */
void interpret_progressive_refinement_end_info( byte* payload, int size, ImageParameters *img ,   h264_decoder* dec_params)
{
 ...
 ************************************************************************
 */
void interpret_motion_constrained_slice_group_set_info( byte* payload, int size, ImageParameters *img,   h264_decoder* dec_params )
{
  ...
  num_slice_groups_minus1   = ue_v("SEI: num_slice_groups_minus1"  , buf,dec_params);
  ...
  {
    slice_group_id   = ue_v("SEI: slice_group_id"  , buf,dec_params);
#ifdef PRINT_MOTION_CONST_SLICE_GROUP_SET_INFO
  ...
  exact_match_flag   = u_1("SEI: exact_match_flag"  , buf,dec_params);
  pan_scan_rect_flag = u_1("SEI: pan_scan_rect_flag"  , buf,dec_params);
									   ...
									   h264_decoder* dec_params )
{
  ...
  seq_parameter_set_id   = ue_v("SEI: seq_parameter_set_id"  , buf,dec_params);
  
  sps = &dec_params->SeqParSet[seq_parameter_set_id];
  ...
  if (sps->vui_parameters_present_flag)
  {
    ...
    if (sps->vui_seq_parameters.nal_hrd_parameters_present_flag)
    {
      ...
      {
        initial_cpb_removal_delay        = u_v(sps->vui_seq_parameters.nal_hrd_parameters.initial_cpb_removal_delay_length_minus1+1, "SEI: initial_cpb_removal_delay"        , buf,dec_params);
        initial_cpb_removal_delay_offset = u_v(sps->vui_seq_parameters.nal_hrd_parameters.initial_cpb_removal_delay_length_minus1+1, "SEI: initial_cpb_removal_delay_offset" , buf,dec_params);
    ...
    if (sps->vui_seq_parameters.vcl_hrd_parameters_present_flag)
    {
      ...
      {
        initial_cpb_removal_delay        = u_v(sps->vui_seq_parameters.vcl_hrd_parameters.initial_cpb_removal_delay_length_minus1+1, "SEI: initial_cpb_removal_delay"        , buf,dec_params);
        initial_cpb_removal_delay_offset = u_v(sps->vui_seq_parameters.vcl_hrd_parameters.initial_cpb_removal_delay_length_minus1+1, "SEI: initial_cpb_removal_delay_offset" , buf,dec_params);
								     ...
								     h264_decoder* dec_params)
{
  ...
  CpbDpbDelaysPresentFlag =  (dec_params->active_sps->vui_parameters_present_flag 
                              && (   (dec_params->active_sps->vui_seq_parameters.nal_hrd_parameters_present_flag != 0)
                                   ||(dec_params->active_sps->vui_seq_parameters.vcl_hrd_parameters_present_flag != 0)));

  if (CpbDpbDelaysPresentFlag )
  {
    if (dec_params->active_sps->vui_parameters_present_flag)
    {
      if (dec_params->active_sps->vui_seq_parameters.nal_hrd_parameters_present_flag)
      {
        cpb_removal_len = dec_params->active_sps->vui_seq_parameters.nal_hrd_parameters.cpb_removal_delay_length_minus1 + 1;
        dpb_output_len  = dec_params->active_sps->vui_seq_parameters.nal_hrd_parameters.dpb_output_delay_length_minus1  + 1;
      }
      else if (dec_params->active_sps->vui_seq_parameters.vcl_hrd_parameters_present_flag)
      {
        cpb_removal_len = dec_params->active_sps->vui_seq_parameters.vcl_hrd_parameters.cpb_removal_delay_length_minus1 + 1;
        dpb_output_len  = dec_params->active_sps->vui_seq_parameters.vcl_hrd_parameters.dpb_output_delay_length_minus1  + 1;
      }
    }
    
    if ((dec_params->active_sps->vui_seq_parameters.nal_hrd_parameters_present_flag)||
      (dec_params->active_sps->vui_seq_parameters.vcl_hrd_parameters_present_flag))
    {
      cpb_removal_delay = u_v(cpb_removal_len, "SEI: cpb_removal_delay" , buf,dec_params);
      dpb_output_delay  = u_v(dpb_output_len,  "SEI: dpb_output_delay"  , buf,dec_params);
#ifdef PRINT_PCITURE_TIMING_INFO
  ...
  {
    picture_structure_present_flag  =  dec_params->active_sps->vui_seq_parameters.pic_struct_present_flag;
  }

  if (picture_structure_present_flag)
  {
    picture_structure = u_v(4, "SEI: pic_struct" , buf,dec_params);
#ifdef PRINT_PCITURE_TIMING_INFO
    ...
    case 8:
      ...
      break;
    default:
      printf("reserved picture_structure used (can't determine NumClockTs)");
	  exit(0);
    }
    for (i=0; i<NumClockTs; i++)
    {
      clock_time_stamp_flag = u_1("SEI: clock_time_stamp_flag"  , buf,dec_params);
#ifdef PRINT_PCITURE_TIMING_INFO
      //printf("clock_time_stamp_flag = %d\n",clock_time_stamp_flag);
#endif
      if (clock_time_stamp_flag)
      {
        ct_type               = u_v(2, "SEI: ct_type"               , buf,dec_params);
        nuit_field_based_flag = u_1(   "SEI: nuit_field_based_flag" , buf,dec_params);
        counting_type         = u_v(5, "SEI: counting_type"         , buf,dec_params);
        full_timestamp_flag   = u_1(   "SEI: full_timestamp_flag"   , buf,dec_params);
        discontinuity_flag    = u_1(   "SEI: discontinuity_flag"    , buf,dec_params);
        cnt_dropped_flag      = u_1(   "SEI: cnt_dropped_flag"      , buf,dec_params);
        nframes               = u_v(8, "SEI: nframes"               , buf,dec_params);
        ...
        {
          seconds_value         = u_v(6, "SEI: seconds_value"   , buf,dec_params);
          minutes_value         = u_v(6, "SEI: minutes_value"   , buf,dec_params);
          hours_value           = u_v(5, "SEI: hours_value"     , buf,dec_params);
#ifdef PRINT_PCITURE_TIMING_INFO
        ...
        {
          seconds_flag          = u_1(   "SEI: seconds_flag" , buf,dec_params);
#ifdef PRINT_PCITURE_TIMING_INFO
          ...
          {
            seconds_value         = u_v(6, "SEI: seconds_value"   , buf,dec_params);
            minutes_flag          = u_1(   "SEI: minutes_flag" , buf,dec_params);
#ifdef PRINT_PCITURE_TIMING_INFO
            //printf("seconds_value = %d\n",seconds_value);
            ...
            {
              minutes_value         = u_v(6, "SEI: minutes_value"   , buf,dec_params);
              hours_flag            = u_1(   "SEI: hours_flag" , buf,dec_params);
#ifdef PRINT_PCITURE_TIMING_INFO
              //printf("minutes_value = %d\n",minutes_value);
              //printf("hours_flag    = %d\n",hours_flag);
#endif    
        ...
        if(dec_params->active_sps->vui_seq_parameters.nal_hrd_parameters.time_offset_length) //!KS which HRD params shall be used?
        {
          time_offset=0;
