...
pic_parameter_set_rbsp_t *AllocPPS ()
{
	...
	if ((p = (pic_parameter_set_rbsp_t *)h264_malloc (sizeof (pic_parameter_set_rbsp_t) * 1)) == NULL)
	{
		printf("AllocPPS: PPS");
		exit(0);
	}
	
    memset(p,0,sizeof(pic_parameter_set_rbsp_t));
	
	if ((p->slice_group_id = (unsigned int *)h264_malloc (SIZEslice_group_id * 1)) == NULL)
	{
		printf("AllocPPS: slice_group_id");
		exit(0);
	}
	return p;
}
...
seq_parameter_set_rbsp_t *AllocSPS ()
 {
   ...
   if ((p = (seq_parameter_set_rbsp_t *)h264_malloc (sizeof (seq_parameter_set_rbsp_t) * 1)) == NULL)
   {
	   printf("AllocSPS: SPS");
	   exit(0);
   }
 ...
 */
 ...
 {
   assert (pps != NULL);
   if (pps->slice_group_id != NULL) h264_free (pps->slice_group_id);
   h264_free (pps);
 }
