...
)
{
  ...
  *------------------------------------------------------------*/
  ...
  for(i=0;  i<NB_POS; i++)
  {
	...
	cor += ((Word32)(*ptr_h1) * (Word32)(*ptr_h1)) << 1; ptr_h1++;
    *p4-- = extract_h(cor);
  ...
  *-----------------------------------------------------------------*/
  ...
  for(k=0; k<NB_POS; k++) {
          ...
          for(i=k+(Word16)1; i<NB_POS; i++ ) {
				  ...
				  cor += ((Word32)(*ptr_h1) * (Word32)(*ptr_h2)) << 1; ptr_h1++; ptr_h2++;
                  //cor = L_mac(cor, *ptr_h1, *ptr_h2); ptr_h1++; ptr_h2++;
				  cor += ((Word32)(*ptr_h1) * (Word32)(*ptr_h2)) << 1; ptr_h1++; ptr_h2++;
...
)
{
  ...
  /* FUNCTION MODIFIED */
     ...
     /* Init pointers */
   rri0i0 = rr;
   rri1i1 = rri0i0 + NB_POS;				// rr[  8]
   rri2i2 = rri1i1 + NB_POS;				// rr[ 16]
   rri3i3 = rri2i2 + NB_POS;				// rr[ 24]
   rri4i4 = rri3i3 + NB_POS;				// rr[ 32]
   rri0i1 = rri4i4 + NB_POS;				// rr[ 40]
   rri0i2 = rri0i1 + MSIZE;					// rr[104]
   rri0i3 = rri0i2 + MSIZE;					// rr[168]
   rri0i4 = rri0i3 + MSIZE;					// rr[232]
   rri1i2 = rri0i4 + MSIZE;					// rr[296]
   rri1i3 = rri1i2 + MSIZE;					// rr[360]
   rri1i4 = rri1i3 + MSIZE;					// rr[424]
   rri2i3 = rri1i4 + MSIZE;					// rr[488]
   rri2i4 = rri2i3 + MSIZE;					// rr[552]
  ...
  for (track=3, trk=0; track<5; track++, trk++)
  {
    ...
    *------------------------------------------------------------------*/
    ...
    for (i=0; i<2; i++)
    {
      ...
      p0 = ptr_rri2i3_i4 + shl(j, 3);			// p0 ---> rr[544]
      p1 = ptr_rri3i3_i4;
      ...
      for (i1=track; i1<L_SUBFR; i1+=STEP)
      {
		...
		alp2 = (Word32)alp1+ ((Word32)(*p0++)<<15);		/* _1_2 = 16384 */
        //alp2 = L_mac(alp2, *p1++, _1_4);
		alp2 = (Word32)alp2+ ((Word32)(*p1++)<<14);	/* _1_4 = 8192 */
    ...
    *------------------------------------------------------------------*/
    ...
    for (i3=1; i3<L_SUBFR; i3+=STEP)
    {
	  ...
	  s = (Word32)s + ((Word32)(*p1)<<14);      
	  p1 += NB_POS;

      //s = L_mac(s, *p2++, _1_8);
	  s = (Word32)s + ((Word32)(*p2++)<<13);      
      *p3++ = round(s);
    }
    ...
    for (i2=0; i2<L_SUBFR; i2+=STEP)
    {
	  ...
	  alp1 = (Word32)alp0+ ((Word32)(*p0)<<13);	
	  p0 += NB_POS;
      //alp1 = L_mac(alp1, *p1, _1_8);       p1 += NB_POS;
	  alp1 = (Word32)alp1 + ((Word32)(*p1)<<13);	
	  p1 += NB_POS;
      //alp1 = L_mac(alp1, *p2++, _1_16);
	  alp1 = (Word32)alp1 + ((Word32)(*p2++)<<12);	/* _1_16 = 2048 */
      ...
      for (i3=1; i3<L_SUBFR; i3+=STEP)
      {
		...
		alp2 = (Word32)alp1+ ((Word32)(*p3++)<<13);	
        //alp2 = L_mac(alp2, *p4++, _1_2);
		alp2 = (Word32)alp2+ ((Word32)(*p4++)<<15);	
    ...
    *------------------------------------------------------------------*/
    ...
    for (i=0; i<2; i++)
    {
      ...
      for (i1=0; i1<L_SUBFR; i1+=STEP)
      {
		...
		alp2 = (Word32)alp1+ ((Word32)(*p0)<<15);	
		p0 += NB_POS;
        //alp2 = L_mac(alp2, *p1++, _1_4);
		alp2 = (Word32)alp2+ ((Word32)(*p1++)<<14);	
