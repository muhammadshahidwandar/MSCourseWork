...
extern void asmCopy( Word16 x[], Word16 y[], Word16 L );
    ...
    *-------------------------------------------------------*/
    ...
    for (i = 0; i < L_SUBFR;  i++)
    {
       ...
       //L_temp = L_mult(exc[i+i_subfr], gain_pitch);
		L_temp = ((Word32)(exc[i+i_subfr])* (Word32)gain_pitch)<<1;
       //L_temp = L_mac(L_temp, code[i], gain_code);
		L_temp += ((Word32)code[i] * (Word32) gain_code)<<1;
       L_temp = L_shl(L_temp, 1);
		//L_temp = (Word32)(L_temp)<<1;
